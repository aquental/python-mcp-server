...
def plot_embeddings(reduced_embeddings, sentences, categories, color_map, shape_map):
    ...
    for i, (sentence, category) in enumerate(zip(sentences, categories)):
        ...
        plt.scatter(
            x, y, color=color_map[category], marker=shape_map[category])
...
if __name__ == "__main__":
    ...
    reduced_embeddings = compute_tsne_embeddings(
        sentences, perplexity=perplexity, max_iter=max_iter)
    plot_embeddings(reduced_embeddings, sentences,
                    categories, color_map, shape_map)
