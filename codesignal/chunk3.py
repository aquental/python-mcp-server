...
def chunk_text(text, chunk_size=10, overlap=2):
    ...
    # (how many words to move forward after each chunk)
    step = max(1, chunk_size - overlap)   # prevents step <= 0

    # Create chunks with overlap
    for i in range(0, len(words), step):
        chunk_words = words[i:i + chunk_size]
        if chunk_words:                       # skip any empty chunk
...
if __name__ == "__main__":
    ...
    chunked_docs = load_and_chunk_dataset(
        dataset_file, chunk_size=10, overlap=2)
