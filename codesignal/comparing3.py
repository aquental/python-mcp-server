...
def bow_search(query, docs):
    ...
    # Transform each document into a BOW vector + compute dot product
    scores = []
    for i, doc in enumerate(docs):
        doc_vec = bow_vectorize(doc)
        score = np.dot(query_vec, doc_vec)          # lexical overlap score
        scores.append((i, score))

    # Sort documents so that higher lexical overlap is first
    scores.sort(key=lambda x: x[1], reverse=True)

    # Return sorted scores (list of (doc_index, score) tuples)
    return scores
...
if __name__ == "__main__":
    ...
    print(f"Query: {query}")
    print(f"Vocabulary size: {len(VOCAB)} (unigrams only)")
