...
def metadata_enhanced_search(query, collection, categories=None, top_k=3):
    ...
    # === NEW: Fallback logic ===
    # If we used a filter AND got zero results → rerun without any filter
    if where_clause is not None and len(results["documents"][0]) == 0:
        print(
            f"⚠️  No results found with strict filter {categories}. Falling back to unfiltered search...")

        results = collection.query(
            query_texts=[query],
            n_results=top_k,
            where=None                    # relaxed / no filter
        )

    # Build a result list from the (possibly fallback) results
    retrieved_chunks = []
...
if __name__ == "__main__":
    ...
    collection = client.create_collection(
        "metadata_demo_collection", embedding_function=embed_func)
    ...
    no_filter_results = metadata_enhanced_search(
        query_input, collection, categories=None, top_k=3)
    for res in no_filter_results:
        print(
            f"Doc ID: {res['doc_id']}, Category: {res['category']}, Distance: {res['distance']:.4f}")
    ...
    print(
        f"======== WITH VERY STRICT CATEGORY FILTER ({very_strict_category}) ========")
    strict_filter_results = metadata_enhanced_search(
        query_input, collection, categories=[very_strict_category], top_k=3)
